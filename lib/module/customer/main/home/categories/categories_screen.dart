...
class CategoriesScreen extends StatelessWidget {
  ...
  SingleChildScrollView _buildMainBody() {
  ...
  Widget _buildListItem(int index) {
    ...
    return InkWell(
      onTap: () async {
        ...
        print("${_Profcontroller.Review[index].data().nameWorker!} 555555");
        Get.toNamed(Routes.profileCategoryRoutes, parameters: {
          "nameWorker": _Profcontroller.Review[index].data().nameWorker!,
          'bio': item.bio!,
          'price': item.price!,
  ...
  Future<void> getWorkerData(item) async {
    ...
    await StorageService.to
        .setString(Constants.STRORAGE_LOCATION_WORKER, item.location!);
    await StorageService.to
        .setString(Constants.STRORAGE_SUB_SERVICE_NAME, item.subService!);
    await StorageService.to.setString(
        Constants.STRORAGE_NAME_USER, UserStore.t0.profile.displayName!);
  }
}
